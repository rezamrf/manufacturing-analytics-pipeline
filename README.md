# Automated Data Pipeline for Manufacturing Analytics using Kestra, Delta Lake, and ClickHouse

## Project Overview
Repositori ini berisi implementasi *end to end data pipeline* yang dirancang untuk kebutuhan analitik pada case kali ini di sektor manufaktur. Fokus utama dari project ini adalah membangun arsitektur *batch processing* yang *scalable*, tangguh, dan efisien untuk memproses data operasional menjadi *insight* bisnis ataupun untuk *memonitoring* di sektor manufaktur.

## System Architecture & Workflow

*(Anda bisa menyisipkan gambar arsitektur di sini, misalnya: `![Architecture Diagram](docs/architecture.png)`)*
...
5. **Data Visualization (Apache Superset):** Superset terhubung langsung ke ClickHouse untuk memvisualisasikan data dan menyediakan *dashboard* pemantauan analitik pabrik.
...
* **Database & Storage:** PostgreSQL, MinIO, ClickHouse
* **Data Format:** Parquet, Delta Lake (`delta-rs`)
...
* **Eliminasi OLTP untuk Analytical Workload:** Menghindari *anti-pattern* dengan tidak memindahkan data analitik dari DWH (ClickHouse) kembali ke *database* baris (seperti MySQL) hanya untuk visualisasi. Superset langsung melakukan *query* ke ClickHouse, memanfaatkan kecepatan *engine* OLAP secara maksimal.

## Repository Structure

```text
├── infrastructure/             # Konfigurasi Docker Compose untuk servis lokal (Postgres, MinIO, ClickHouse, Kestra, Superset)
